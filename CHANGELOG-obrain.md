...
All notable changes to Obrain, for future reference (and enjoyment).
...
- **Read-only open mode**: `ObrainDB::open_read_only()` uses a shared file lock so multiple processes can read the same `.obrain` file concurrently; mutations are rejected at the session level
...
- **Prometheus metrics export** (`metrics` feature): `MetricsRegistry::to_prometheus()` renders all counters, gauges, and histograms in Prometheus text exposition format, ready for `/metrics` HTTP endpoints; `ObrainDB::metrics_prometheus()` provides one-call access; `snapshot_with_cache()` merges plan cache hit/miss/size stats into the snapshot
- **Tracing spans**: structured `tracing` spans on query and transaction lifecycle boundaries (`obrain::session::execute`, `obrain::query::parse`, `obrain::query::optimize`, `obrain::query::plan`, `obrain::query::execute`, `obrain::tx::begin`, `obrain::tx::commit`, `obrain::tx::rollback`); zero-cost when no subscriber is registered
...
- **Unsafe code enforcement**: `#![forbid(unsafe_code)]` on pure-safe crates (obrain, obrain-adapters, bindings-common, python, wasm), `#![deny(unsafe_code)]` on crates with targeted unsafe (obrain-common, obrain-core, obrain-engine, obrain-cli)
...
- **RDF code consolidation**: consolidated scattered RDF `#[cfg]` gates in `obrain-engine` by extracting dedicated `database/rdf_ops.rs` and `session/rdf.rs` modules
...
- **ObrainDB graph context**: one-shot `db.execute()` calls now persist `USE GRAPH` context across calls; `current_graph()` and `set_current_graph()` public API for programmatic access
...
- **Plan cache invalidation**: query plan cache is automatically cleared after DDL operations (CREATE/DROP INDEX, TYPE, CONSTRAINT, etc.), with manual `clear_plan_cache()` API on `ObrainDB` and `Session`
...
- **Examples CI**: added `cargo build -p obrain-examples` to CI checks
...
- **Pluggable storage**: `GraphStore`/`GraphStoreMut` traits decouple all query operators and algorithms from `LpgStore`. Use `ObrainDB::with_store(Arc<dyn GraphStoreMut>, Config)` to plug in any backend
...
- **`obrain-bindings-common` crate**: shared entity extraction, error classification and JSON conversion for all four bindings (Python, Node.js, C, WASM)
...
- **Native SSSP procedure**: `CALL obrain.sssp('node_name', 'weight')` for LDBC Graphanalytics compatibility
...
- **Query error quality**: translator errors now produce `QueryError` with semantic error codes (`OBRAIN-Q002`) instead of generic internal errors. More actionable messages
...
- **CALL procedure support**: invoke any of the 22 built-in graph algorithms from query strings: `CALL obrain.<algorithm>() [YIELD columns]`. Supported in GQL, Cypher and SQL/PGQ
- **Map literal arguments**: `CALL obrain.pagerank({damping: 0.85, max_iterations: 20})`
- **Procedure listing**: `CALL obrain.procedures()` returns all available procedures

## [0.5.1] - 2026-02-12

Hybrid search, built-in embeddings, change data capture. The features that make obrain-memory work.
...
- **Standardized error codes**: all errors carry `OBRAIN-XXXX` codes (Q = query, T = transaction, S = storage, V = validation, X = internal) with `error_code()` and `is_retryable()`
...
- **Python CLI removed**: replaced by the unified `obrain-cli` Rust binary
...
Obrain now runs in the browser. WebAssembly bindings with TypeScript definitions at 660 KB gzipped.

### Added

- **WebAssembly bindings** (`@obrain-db/wasm`): `execute()`, `executeRaw()`, `nodeCount()`, `edgeCount()`, full TypeScript definitions. 660 KB gzipped (target was <800 KB)
- **Feature-gated platform subsystems**: `parallel`, `spill`, `mmap`, `wal` are opt-in, making wasm32 compilation straightforward

## [0.4.1] - 2026-02-08

Go and C bindings. Obrain now embeds in pretty much any language.
...
- **"Did you mean?" suggestions**: typo in a variable or label? Obrain suggests the closest match
...
- **CLI** (`obrain-cli`): inspect, backup, export, manage WAL, compact databases
...
- Renamed from Graphos to Obrain, reset version to 0.1.0
