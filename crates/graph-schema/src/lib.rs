...
pub fn discover_schema(store: &LpgStore) -> GraphSchema {
        ...
        let find_best = |candidates: &[&str]| -> Option<String> {
            candidates
                ...
                .filter_map(|&c| {
                    ...
                    if freq > sample_size / 3 {
                        Some((c.to_string(), freq))
                    } else {
                        None
                    }
                })
                ...
                .collect::<Vec<_>>()
            {
                if let Some(tnode) = store.get_node(target) {
                    ...
                    if !etype.is_empty() && !tlabel.is_empty() {
                        *edge_patterns
                            .entry((label.clone(), etype, tlabel))
                            .or_default() += 1;
                    }
                }
            }
    ...
    for (label, count) in &label_infos {
        let node_ids = store.nodes_by_label(label);
        let sample_size = node_ids.len().min(100);
        let total_degree: usize = node_ids[..sample_size]
            .iter()
            .map(|&nid| store.neighbors(nid, Direction::Both).count())
            .sum();
        let avg_degree = if sample_size > 0 {
            total_degree as f64 / sample_size as f64
        } else {
            0.0
        };
    ...
    labels_with_importance.sort_by(|a, b| {
        b.importance
...
pub fn fuzzy_match_label(term: &str, schema: &GraphSchema) -> Option<(String, f64)> {
    ...
    for info in &schema.labels {
        ...
        let candidates = vec![
            ...
            if label_lower.ends_with('s') {
                label_lower[..label_lower.len() - 1].to_string()
            } else {
                String::new()
            },
        ];
        ...
        for candidate in &candidates {
            ...
            if candidate.starts_with(&term_lower) || term_lower.starts_with(candidate.as_str()) {
                let sim = term_lower.len().min(candidate.len()) as f64
                    / term_lower.len().max(candidate.len()) as f64;
                if sim > 0.6 {
...
pub fn get_node_name_generic(
    store: &LpgStore,
    node_id: NodeId,
    props: Option<&DisplayProps>,
) -> String {
    store
        .get_node(node_id)
        .and_then(|n| {
            if let Some(dp) = props {
                if let Some(ref field) = dp.name_field {
                    return n
                        .properties
                        .get(&PropertyKey::from(field.as_str()))
                        .and_then(|v| v.as_str())
                        .filter(|s| !s.is_empty())
                        .map(|s| s.to_string());
                }
...
fn collect_outgoing_relations(
    ...
    for (target_id, edge_id) in store
        ...
        .collect::<Vec<_>>()
    {
        ...
        if etype.starts_with("__") || etype == "HAS_PROPERTY" {
            continue;
        }
    ...
    if name.is_empty() && desc.is_empty() {
        for (key, val) in node.properties.iter() {
            if let Some(s) = val.as_str() {
                ...
                if is_internal_key(ks) || ks == "embedding_model" {
                    continue;
                }
...
mod tests {
    ...
    fn full_budget_renders_all_non_internal_props() {
        let (store, nid) = make_store_with_node(
            "Task",
            &[
                ("name", "Fix login bug"),
        ...
        let (text, _labels, _summary) =
            extract_node_with_budget(&store, nid, &schema, TextBudget::Full);

        // Should contain extra props alphabetically
        assert!(
            text.contains("assigned_to: alice"),
            "missing assigned_to: {text}"
        );
        assert!(
            text.contains("category: backend"),
            "missing category: {text}"
        );
        assert!(
            text.contains("due_date: 2026-04-01"),
            "missing due_date: {text}"
        );
        assert!(text.contains("priority: high"), "missing priority: {text}");
        assert!(
            text.contains("severity: critical"),
            "missing severity: {text}"
        );
        ...
        assert!(
            !text.contains("hash: abc123"),
            "internal hash leaked: {text}"
        );
    ...
    fn compact_budget_renders_only_3_props() {
        ...
        assert_eq!(
            pipe_count, 3,
            "compact should have exactly 3 extra props, got {pipe_count}: {text}"
        );
    ...
    fn minimal_budget_renders_only_name() {
        ...
        assert!(
            !text.contains("Users cannot log in"),
            "desc in minimal: {text}"
        );
    ...
    fn internal_keys_are_excluded() {
        ...
        assert!(
            text.contains("visible_prop: yes"),
            "visible prop missing: {text}"
        );
    ...
    fn long_property_values_are_truncated_at_50_chars() {
        ...
        let (store, nid) =
            make_store_with_node("Doc", &[("name", "My Document"), ("long_field", &long_val)]);
        let schema = discover_schema(&store);
        let (text, _, _) = extract_node_with_budget(&store, nid, &schema, TextBudget::Full);
        ...
        if let Some(idx) = text.find("long_field: ") {
            ...
            assert!(
                rendered_val.chars().count() <= 53, // 50 + "..." (3 chars)
                "value not truncated: len={}, val='{rendered_val}'",
                rendered_val.chars().count()
            );
            assert!(
                rendered_val.contains("..."),
                "no truncation marker: {rendered_val}"
            );
        } else {
    ...
    fn backward_compat_wrapper_uses_full_budget() {
        let (store, nid) =
            make_store_with_node("Task", &[("name", "Test"), ("visible_prop", "yes")]);
        let schema = discover_schema(&store);
