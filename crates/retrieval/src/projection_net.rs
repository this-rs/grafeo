...
impl ProjectionNet {
    ...
    pub fn forward(&self, input: &[f32]) -> Result<Vec<f32>> {
        if input.len() != self.n_input {
            bail!("ProjectionNet::forward: input.len()={} but n_input={}", input.len(), self.n_input);
        }
    ...
    pub fn train(
        ...
        for epoch in 0..epochs {
            ...
            for (input, target) in data {
                ...
                self.backward_sgd(input, &h1_pre, &h1, &h2_pre, &output, target, lr, lambda_cosine);
                self.n_updates += 1;
            }
            ...
            if epoch % 10 == 0 || epoch == epochs - 1 {
                eprintln!("  [ProjectionNet] epoch {}/{}: loss={:.6}", epoch + 1, epochs, avg_loss);
            }
        }
    ...
    fn forward_with_intermediates(&self, input: &[f32]) -> (Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>) {
    ...
    fn backward_sgd(
        ...
        for i in 0..n {
            ...
            let d_cos = -(target[i] / (norm_o * norm_t) - output[i] * dot / (norm_o * norm_o * norm_o * norm_t));
        ...
        let d_h2: Vec<f32> = d_output.iter().enumerate()
        ...
        let d_h1_pre: Vec<f32> = d_h1.iter().zip(h1_pre).zip(h1)
    ...
    pub fn train_contrastive(
        ...
        for epoch in 0..epochs {
            ...
            let projected: Vec<Vec<f32>> = data.iter()
                .map(|(input, _)| self.forward(input).unwrap_or_else(|_| vec![0.0; self.n_hidden]))
                .collect();
            ...
            for (_idx, (input, target)) in data.iter().enumerate() {
                ...
                let mse = mse_loss(&output, target);
                let cos_loss = 1.0 - cosine_similarity(&output, target);
                let margin_loss = (config.cosine_margin - cosine_similarity(&output, target)).max(0.0);
                let recon_loss = mse + lambda_cosine * cos_loss + config.lambda_margin * margin_loss;
                ...
                if cosine_similarity(&output, target) < config.cosine_margin {
                    ...
                    self.apply_output_gradient(input, &h1_pre, &h1, &h2_pre, &cos_grad, lr * config.lambda_margin, true);
                }
            ...
            for &(anchor_idx, pos_idx, ref neg_indices) in contrastive_pairs {
                ...
                let sim_negs: Vec<f32> = neg_indices.iter()
                    .filter(|&&ni| ni < projected.len())
                    .map(|&ni| crate::contrastive::cosine_sim(anchor_emb, &projected[ni]))
                    .collect();

                if sim_negs.is_empty() { continue; }

                let (loss, d_sim_pos, d_sim_negs) = crate::contrastive::info_nce_loss(
                    sim_pos, &sim_negs, config.temperature,
                );
                ...
                if anchor_input.len() != self.n_input { continue; }
                ...
                for (k, &ni) in neg_indices.iter().enumerate() {
                    if ni >= projected.len() || k >= d_sim_negs.len() { continue; }
                    let grad_neg = crate::contrastive::cosine_sim_grad_a(anchor_emb, &projected[ni]);
                    for i in 0..self.n_hidden {
                ...
                self.apply_output_gradient(
                    anchor_input, &h1_pre, &h1, &h2_pre, &d_anchor,
                    lr * config.lambda_topo, false,
                );
            }
            ...
            if epoch % 10 == 0 || epoch == epochs - 1 {
                eprintln!("  [ProjectionNet] epoch {}/{}: total={:.6} recon={:.6} contra={:.6}",
                    epoch + 1, epochs, avg_total, avg_recon, avg_contra);
            }
        }
    ...
    /// Input dimension.
    pub fn n_input(&self) -> usize { self.n_input }
...
fn mse_loss(output: &[f32], target: &[f32]) -> f32 {
    ...
    output.iter().zip(target).map(|(a, b)| {
        let d = a - b;
        d * d
    }).sum::<f32>() / n
}
...
mod tests {
    ...
    fn test_train_converges() {
        ...
        let data: Vec<(Vec<f32>, Vec<f32>)> = (0..20).map(|i| {
        ...
        assert!(last_loss < 0.5, "final loss should be < 0.5, got {}", last_loss);
    }
    ...
    fn test_train_contrastive_loss_decreases() {
        ...
        let data: Vec<(Vec<f32>, Vec<f32>)> = vec![
            ...
            (vec![1.0, 0.5, -0.3, 0.8, 0.1, -0.1], vec![1.0, 0.5, -0.3, 0.8]),
            (vec![0.9, 0.6, -0.2, 0.7, 0.15, -0.05], vec![0.9, 0.6, -0.2, 0.7]),
            (vec![1.1, 0.4, -0.4, 0.9, 0.05, -0.15], vec![1.1, 0.4, -0.4, 0.9]),
            // Cluster B: nodes 3, 4, 5 (different patterns)
            (vec![-0.5, 1.0, 0.8, -0.3, -0.1, 0.2], vec![-0.5, 1.0, 0.8, -0.3]),
            (vec![-0.4, 0.9, 0.7, -0.2, -0.15, 0.25], vec![-0.4, 0.9, 0.7, -0.2]),
            (vec![-0.6, 1.1, 0.9, -0.4, -0.05, 0.15], vec![-0.6, 1.1, 0.9, -0.4]),
        ];
        ...
        let history = net.train_contrastive(&data, &contrastive_pairs, 50, 0.001, 0.1, &config).unwrap();

        assert!(!history.is_empty());
        let (first_total, _, _) = history[0];
        let (last_total, _, _) = history[history.len() - 1];
        eprintln!("  contrastive: first={:.4}, last={:.4}", first_total, last_total);
        assert!(last_total < first_total, "total loss should decrease: {} → {}", first_total, last_total);
    ...
    fn test_save_load_roundtrip() {
        ...
        let data: Vec<(Vec<f32>, Vec<f32>)> = (0..10).map(|i| {
            let s = (i as f32 + 1.0) * 0.1;
            (vec![s, -s, s*0.5, -s*0.5, 0.1, -0.1, s*0.3, -s*0.3, 0.2, -0.2, s*0.1, -s*0.1],
        ...
        let missing = ProjectionNet::load(std::path::Path::new("/tmp/nonexistent_pnet.bin")).unwrap();
