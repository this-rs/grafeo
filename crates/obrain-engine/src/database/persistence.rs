...
mod tests {
    ...
    // T17 W3c slice 3 (2026-04-22): the two v2-native-format lifecycle tests
    // — `test_native_v2_lifecycle` and `test_native_v2_hnsw_persistence` —
    // were deleted alongside the producer they exercised. Both drove
    // `save_as_obrain_file` (v2 writer) → reopen → equivalence, but slice 2
    // stubbed `save_as_obrain_file` to return `Error::Internal` (the writer
    // silently produced 0-node files in substrate mode) and slice 3 deleted
    // `write_native_v2` + `write_native_v2_inner` + the CSR/string-table/
    // values machinery entirely. The remaining envelope codec is tested
    // inside `native_writer::tests` (HNSW envelope roundtrip).
    //
    // Read-path coverage for legacy `.obrain` files still exists in
    // `crates/obrain-migrate` (full upgrade tests) and the remaining
    // integration tests in `tests/obrain_file.rs`. T17b will add a new
    // lifecycle test against `SubstrateStore::snapshot_to_path()` once that
    // producer lands.
}
