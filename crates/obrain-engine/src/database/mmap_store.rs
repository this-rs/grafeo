...
// T17 W3c slice 3: the `test_mmap_store_equivalence` test used
// `super::super::native_writer::write_native_v2(&store, &path, 42, 100)` to
// materialize an `LpgStore` into a v2 `.obrain` file, then opened that file
// through `MmapStore` and round-tripped every query. Slice 3 deleted the
// native v2 producer (`write_native_v2`, `write_native_v2_inner`, the CSR
// builders, etc.) — the test is now structurally impossible.
//
// `MmapStore`'s READ path is still exercised end-to-end by `obrain-migrate`
// (every v1/v2 `.obrain` file round-trips through `MmapStore` on upgrade),
// and by the remaining engine integration tests in `tests/obrain_file.rs`.
// When T17b reintroduces a real snapshot producer via
// `SubstrateStore::snapshot_to_path()`, this equivalence test (or its
// substrate-native successor) should return.
