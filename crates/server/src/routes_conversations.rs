...
pub async fn list_conversations(
    ...
    match state.actor.list_conversations().await {
        Ok(convs) => {
            let data = convs.into_iter().map(|c| Conversation {
            ...
            (StatusCode::OK, Json(serde_json::to_value(ConversationList {
                object: "list".to_string(),
                data,
            }).unwrap())).into_response()
        }
        Err(e) => error_response(StatusCode::INTERNAL_SERVER_ERROR, &format!("{e}")).into_response(),
    }
}
...
pub async fn create_conversation(
    ...
    match state.actor.create_conversation(title.clone()).await {
        Ok(info) => {
            ...
            (StatusCode::CREATED, Json(serde_json::to_value(conv).unwrap())).into_response()
        }
        Err(e) => error_response(StatusCode::INTERNAL_SERVER_ERROR, &format!("{e}")).into_response(),
    }
}
...
pub async fn get_conversation(
    ...
    let conv_id = match parse_conv_id(&conv_id_str) {
        Some(id) => id,
        None => return error_response(StatusCode::BAD_REQUEST, "Invalid conversation ID format").into_response(),
    };
...
pub async fn delete_conversation(
    ...
    match state.actor.delete_conversation(conv_id).await {
        ...
        Ok(false) => error_response(StatusCode::NOT_FOUND, "Conversation not found").into_response(),
        Err(e) => error_response(StatusCode::INTERNAL_SERVER_ERROR, &format!("{e}")).into_response(),
    }
}
...
pub async fn list_items(
    ...
    match state.actor.get_messages(conv_id, 100).await {
        Ok(msgs) => {
            let data: Vec<ConversationItem> = msgs.into_iter().enumerate().map(|(i, m)| {
            ...
            (StatusCode::OK, Json(serde_json::to_value(ConversationItemList {
                object: "list".to_string(),
                data,
            }).unwrap())).into_response()
        }
...
pub async fn add_items(
    ...
    for item in &req.items {
        let content_text = item.content.iter()
            .map(|c| c.text.as_str())
            .collect::<Vec<_>>()
            .join("\n");

        if let Err(e) = state.actor.add_message(conv_id, item.role.clone(), content_text).await {
            return error_response(StatusCode::INTERNAL_SERVER_ERROR, &format!("{e}")).into_response();
        }
    }
