...
class BaseAdvancedQueriesTest(ABC):
    ...
    def setup_social_graph(self, db):
        """Set up: Alice(30,NYC), Bob(25,LA), Carol(35,London) with KNOWS edges."""
    ...
    def test_unwind_with_match(self, db):
        ...
        result = self.execute_query(
            db, "MATCH (n:Person {name: 'Alice'}) UNWIND [10, 20] AS x RETURN n.name, x"
        )
        rows = list(result)
        assert len(rows) == 2
    ...
    def test_merge_match_existing(self, db):
        ...
        result = self.execute_query(
            db, "MERGE (n:Person {name: 'Alice'}) RETURN n.name"
        )
        rows = list(result)
        assert len(rows) == 1
    ...
    def test_optional_match_null(self, db):
        ...
        result = self.execute_query(
            db,
            "MATCH (a:Person {name: 'Alice'}) OPTIONAL MATCH (a)-[:MANAGES]->(c) RETURN a, c",
        )
        rows = list(result)
    ...
    def test_null_property_access(self, db):
        self.setup_social_graph(db)
        result = self.execute_query(
            db, "MATCH (n:Person {name: 'Alice'}) RETURN n.nonexistent"
        )
        rows = list(result)
        assert len(rows) == 1
